...
atLeastOne (REString f1) (REString f2) = REString $ "((" ++ f1 ++ "(" ++ f2 ++ ")?" ++ ")|(" ++ f2 ++ "))" -- !
...
noNumber :: Int -> MarkableRE
noNumber n = reSimple $ "[^.0-9]{0," ++ (show n) ++ "}"
...
date1 :: MarkableRE
date1 = opt (dayOfWeek `plus` (noNumber numFuzzySpace))
		`plus`
		singleYear
...
date2 = dayOfWeek
		`plus`
		(opt ((noNumber numFuzzySpace) `plus` singleYear))
-- ! tokenize_
date3 :: MarkableRE
date3 = ( 
			(	
				(opt (dayOfWeek `plus` (noNumber numFuzzySpace))) -- !
				`plus`
				(
					(
--						opt 
						...
						`plus` 
						(monthWritten `plus` (opt $ noNumber numFuzzySpace))  -- Monat: nicht optional!
					)
					`exactlyOne`
					(
...
theDateExpression = date1
{-((date4 `exactlyOne` date3 `exactlyOne` date2 `exactlyOne` date1) `plus` opt ((noNumber numFuzzySpace) `plus` time)) 
					`exactlyOne` 
					(opt (time `plus` (noNumber numFuzzySpace)) `plus` (date4 `exactlyOne` date3 `exactlyOne` date2 `exactlyOne` date1))
-}
{-
test :: String
