...
instance XmlPickler PageInfo where
  xpickle = xpWrap (fromTuple, toTuple) xpFunction
    where
      fromTuple (m, a, c, d) = PageInfo m a c d
      toTuple (PageInfo m a c d) = (m, a, c, d)
...
getTeaserTextDates dateExtractor dateProcessor =
                   getHtmlPlainText
                   >>^
                   (words >>> unwords >>> (getContextFunc dateProcessor . dateExtractor) >>> toJSONArray)
...
getTeaserTextPageCont = getHtmlText
                        >>^
                        (words >>> unwordsCont 50) -- take the first 50 words from content
...
getTeaserTextDates dateExtractor dateProcessor =
                  ...
                  >. (concat >>> normalizeWS)
                  >>^
                  dateExtractor >>> getContextFunc dateProcessor
                  >>>
                  toJSONArray
...
getTeaserTextCalender dateExtractor dateProcessor =
                      (
                        ...
                        >>^
                        (\ (x, y) ->
                            let res = (getNormFunc D.dateRep2NormalizedDates . dateExtractor $ x) in
                            [["Fh-Wedel Kalender:"
