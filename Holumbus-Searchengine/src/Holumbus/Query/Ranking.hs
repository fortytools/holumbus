...
rank (RankConfig fd fw {-ld lw-}) r = Result scoredDocHits scoredWordHits
  where
  scoredDocHits = IM.mapWithKey (\k (di, dch) -> (setDocScore (fd k di dch) di, dch)) $ docHits r
...
docRankByCount _ _ h = fromIntegral $ M.fold (\h1 r1 -> M.fold (\h2 r2 -> IS.size h2 + r2) r1 h1) 0 h

-- | Rank words by count.
wordRankByCount :: Word -> WordInfo -> WordContextHits -> Score
wordRankByCount _ _ h = fromIntegral $ M.fold (\h1 r1 -> IM.fold ((+) . IS.size) r1 h1) 0 h
