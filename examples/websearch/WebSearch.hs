...
import Network.CGI         -- For decoding URL-encoded strings
...
websearchService mix = proc inTxn -> do
  ...
  if Prelude.null parseResult then do
    ...
    xmlStatus <- (pickleStatusResult >>> writeString) -< (status, emptyResult)
    setVal _transaction_http_response_body xmlStatus -<< outTxn    
      ...
      if (snd . head) parseResult == "" then do
        ...
        xmlStatus <- (pickleStatusResult >>> writeString) -< (status, rankedResult)
        setVal _transaction_http_response_body xmlStatus -<< outTxn   
        else do
          status <- arr $ setMessage ("Could not parse query: " ++ ((snd . head) parseResult)) -<< status
          xmlStatus <- (pickleStatusResult >>> writeString) -< (status, emptyResult)
          setVal _transaction_http_response_body xmlStatus -<< outTxn   
  where
