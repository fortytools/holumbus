      ...
      >>> ( ( documentStatusOk
	      ...
	      robotsNoIndex'
		    ...
		    `withDefault` []
                  ...
                  strictA
                  >>>
                  traceValue 1 (\ l ->  unwords ["found", show . length $ l, "references"])
                )
              )
              >>^ (\ (a,(b,c)) -> (a,b,c))
          )
...
replaceBaseElement base
    = processChildren ( replaceInHtml
                        `when`
                        (isElem >>> hasName "html")
                      )
    where
    replaceInHtml
        = processChildren ( replaceInHead
                            `when`
                            (isElem >>> hasName "head")
                          )
    replaceInHead
        = processChildren ( none
                            `when`
                            (isElem >>> hasName "base")
                          )                                             -- remove old base element, if there
          >>>
          replaceChildren ( getChildren
                            <+>
                            aelem "base" [sattr "href" base]
                          )                                             -- insert new base element
