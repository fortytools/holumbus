...
module Crawl2.Examples2.CommonStandalone
where
...
type ResultState = CrawlerState PlainDocs
...
crawlerInitState  = initCrawlerState []
...
crawlerConfig	(follow,nofollow) num = addReadAttributes  defaultOpts				-- at the moment no more read attributes are neccessary
  ...
  where
    ...
    --collectHeads	= rnfA . listA $ theHeadings -- force complete evaluation of the result: this is essential, don't delete rnfA
...
--    followRefs		= const True						-- all hrefs are collected
    ...
    theHeadings		  :: IOSArrow XmlTree PlainDoc
    theHeadings     = rawDocF -- multi (hasName "h1") /> getText
      where
      rawDocF :: IOSArrow XmlTree PlainDoc      
      rawDocF = ( listA contextFs &&& titleF &&& customF ) >>^ (\ (x3, (x2, x1)) -> (x3, x2, x1))
      ...
      contextFs     = catA . map contextF $ ixcs -- [ ixDefault {ixc_name    = "headlines", ixc_collectText = getAllText getHeadlines} ]   -- collect all contexts
...
--getDivCol2, getLecture, getBody, getHeadlines :: ArrowXml a => a XmlTree XmlTree
getDivCol2, getBody, getHeadlines :: ArrowXml a => a XmlTree XmlTree
...
mergeStates' first' second' = CrawlerState {
