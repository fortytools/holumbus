...
module Holumbus.Common.MRBinary (
...
instance Binary Int where
    put i   = {-# SCC "putint" #-}put (fromIntegral i :: Int64)
...
instance (Binary a, Binary b) => Binary (a,b) where
    put (a,b)           = {-# SCC "puttupple" #-}put a >> put b
    get                 = liftM2 (,) get get
...
instance Binary a => Binary [a] where
  put []     = {-# SCC "putemptylist" #-} putWord8 0
  put (x:xs) = {-# SCC "putlist" #-}putWord8 1 >> put x >> put xs
  get = do
    next <- getWord8
    if next==0
      then return []
...
instance Binary Double where
    put d = {-# SCC "putdouble" #-}put (decodeFloat d)
