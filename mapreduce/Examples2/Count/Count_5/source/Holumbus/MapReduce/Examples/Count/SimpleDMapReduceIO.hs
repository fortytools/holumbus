...
client m r a (splitters,mappers,reducers) lss = do
      ...
      mapM_ (\(fn,content) -> FS.createFile fn content fs) files
...
prepareFiles :: Binary a => [[a]] -> Int -> ([(String,B.ByteString)],[String]) -> ([(String,B.ByteString)],[String])
prepareFiles []     _ l = l
prepareFiles (x:xs) i (files,filenames) = prepareFiles xs (i+1) ((fn,bin):files,fn:filenames)
  where
